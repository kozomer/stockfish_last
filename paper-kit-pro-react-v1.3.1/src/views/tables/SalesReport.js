...
*/
  ...
  setPrice(row.price);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        // Convert start-end dates to the desired format
        let formattedStartDate = startDate;
        let formattedEndDate = endDate;
        if (filterOption === "monthly") {
          formattedStartDate = startDate.replace("/", "-") + "-01";
          formattedEndDate = endDate.replace("/", "-") + "-01";
        } else if (filterOption === "yearly") {
          formattedStartDate = startDate + "-01-01";
          formattedEndDate = endDate + "-12-31";
        } else if (filterOption === "daily") {
          formattedStartDate = startDate.replace("/", "-");
          formattedEndDate = endDate.replace("/", "-");
        }
  ...
  <FormGroup>
    ...
    <Input type="select" name="select" id="selectType" onChange={(e) => setType(e.target.value)}>
